...
```shell
git clone https://github.com/scopedb/scopedb-sdk.git
docker compose -f ./scopedb-sdk/dev/docker-compose.yml up
```

Then, you can connect the playground with the command-line tool:

```shell
docker run -it --rm --entrypoint /bin/scopeql --network host scopedb/scopedb:edge
```
